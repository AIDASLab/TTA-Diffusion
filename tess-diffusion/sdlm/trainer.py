...
from sdlm.pipelines.simplex_ddpm import SimplexDDPMPipeline, ModifiedSimplexDDPMPipeline
...
from transformers import AdamW, AutoModelForTokenClassification, RobertaForSequenceClassification
import shutil
from transformers.trainer import TRAINER_STATE_NAME
from sdlm.models import Classifier_Tree, Classifier_POS, RobertaForPreTraining, RobertaForPreTrainingTopic
...
class DiffusionTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):            
            # pipeline.scheduler.reset_token_timesteps()
...
class DiffusionTrainerForControl(Trainer):
    def __init__(
        self,
        noise_scheduler,
        inference_noise_scheduler,
        ...
        **kwargs,
        ...
        if self.decoding_args.classifier_control == "pos":
            #self.classifier = AutoModelForTokenClassification.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
            self.classifier = Classifier_POS.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
        elif self.decoding_args.classifier_control == "length":
            self.classifier = None
        elif self.decoding_args.classifier_control in ["tree", "span"]:
            self.classifier = Classifier_Tree.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
        elif self.decoding_args.classifier_control == "sentiment":
            # try:
            #     self.classifier = RobertaForSequenceClassification.from_pretrained(
            #         decoding_args.classifier_model_name_or_path
            #     ).to("cuda")
            # except:
            #     self.classifier = RobertaForPreTraining.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
            if decoding_args.classifier_model_name_or_path is not None:
                if "checkpoint" in decoding_args.classifier_model_name_or_path or "new" in decoding_args.classifier_model_name_or_path:
                    self.classifier = RobertaForPreTraining.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
                else:
                    self.classifier = RobertaForSequenceClassification.from_pretrained(
                        decoding_args.classifier_model_name_or_path
            ...
            if self.decoding_args.sub_classifiers is not None:
                self.sub_classifiers = []
                for classifier_name in self.decoding_args.sub_classifiers:
                    if "checkpoint" in classifier_name or "new" in classifier_name:
                        sub_classifier = RobertaForPreTraining.from_pretrained(classifier_name).to("cuda")
                    else:
                        sub_classifier = RobertaForSequenceClassification.from_pretrained(
                            classifier_name
                        ).to("cuda")
                    self.sub_classifiers.append({"name": classifier_name, "classifier": sub_classifier})                
            else:
                self.sub_classifiers = None 
        elif self.decoding_args.classifier_control == "topic":
            if decoding_args.classifier_model_name_or_path is not None:            
                self.classifier = RobertaForSequenceClassification.from_pretrained(
                    decoding_args.classifier_model_name_or_path
                ).to("cuda")
            ...
            if self.decoding_args.sub_classifier is not None:
                self.sub_classifier = RobertaForSequenceClassification.from_pretrained(
                    decoding_args.sub_classifier
                ).to("cuda")
            else:
                self.sub_classifier = None
            
            if self.decoding_args.sub_classifiers is not None:
                self.sub_classifiers = [{"name": classifier, "classifier": RobertaForSequenceClassification.from_pretrained(classifier).to("cuda")} for classifier in self.decoding_args.sub_classifiers]
            else:
                self.sub_classifiers = None
                
            #self.classifier = RobertaForPreTrainingTopic.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
        elif self.decoding_args.classifier_control == "toxicity":
            # self.classifier = RobertaForSequenceClassification.from_pretrained(
            #     decoding_args.classifier_model_name_or_path
            # ).to("cuda")      
            self.classifier = RobertaForSequenceClassification.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
        
        elif self.decoding_args.classifier_control == "qqp":
            self.classifier = RobertaForSequenceClassification.from_pretrained(decoding_args.classifier_model_name_or_path).to("cuda")
            print("qqp classifier")
            print(self.classifier)
        else:
            self.classifier = None
        
        if self.decoding_args.sub_classifier is None:
            self.sub_classifier = None
        
        if self.decoding_args.sub_classifiers is None:
            self.sub_classifiers = None
        
        if self.decoding_args.fluency_classifier_name_or_path is not None:
            self.fluency_classifier = RobertaForSequenceClassification.from_pretrained(
                decoding_args.fluency_classifier_name_or_path
            ).to("cuda")
        else:
            self.fluency_classifier = None
    ...
    def evaluation_loop(
        ...
        pipeline = ModifiedSimplexDDPMPipeline(
            model=model,
            scheduler=self.inference_noise_scheduler,
            simplex_value=self.diffusion_args.simplex_value,
            top_p=self.diffusion_args.top_p,
            top_k =self.diffusion_args.top_k,
            sampling_type=self.diffusion_args.sampling_type,
            is_conditional_generation=is_conditional_generation,
            tokenizer=self.tokenizer,
            classifier_free_uncond_input=self.diffusion_args.classifier_free_uncond_input,
            temperature=self.diffusion_args.temperature,
            guidance_softmax_combination=self.diffusion_args.guidance_softmax_combination,
            classifier_control = self.decoding_args.classifier_control,
            classifier_model = self.classifier,
            control_lambda = self.decoding_args.control_lambda,
            control_iteration = self.decoding_args.control_iteration,
            eval_per_n_step = self.decoding_args.eval_per_n_step,
            lambda_schedule = self.decoding_args.lambda_schedule,
            lambda_alpha = self.decoding_args.lambda_alpha,
            lambda_beta = self.decoding_args.lambda_beta,
            sub_classifier = self.sub_classifier,
            sub_classifiers = self.sub_classifiers,
            scale_initial_noise = self.decoding_args.scale_initial_noise,
            scale_intermediate_noise = self.decoding_args.scale_intermediate_noise,
            classifier_begin_step= self.decoding_args.classifier_begin_step,
            classifier_end_step= self.decoding_args.classifier_end_step,
            classifier_interval = self.decoding_args.classifier_interval,
            initial_noise_type = self.decoding_args.initial_noise_type,
            intermediate_noise_type = self.decoding_args.intermediate_noise_type,
            token_schedule = self.decoding_args.token_schedule,
            schedule_warm_up_step = self.decoding_args.schedule_warm_up_step,
            fluency_classifier = self.fluency_classifier,
            fluency_classifier_begin_step = self.decoding_args.fluency_classifier_begin_step,
            fluency_classifier_end_step = self.decoding_args.fluency_classifier_end_step,
            fluency_classifier_interval = self.decoding_args.fluency_classifier_interval,
            fluency_classifier_lambda = self.decoding_args.fluency_classifier_lambda,
            fluency_classifier_iteration = self.decoding_args.fluency_classifier_iteration,
        )
...
class ProgressiveDistillationTrainer(DiffusionTrainer):
    def __init__(self, teacher_model, teacher_timestep_count, student_timestep_count, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.teacher_model = teacher_model
        self.teacher_model.eval()  # Freeze the teacher
        for param in self.teacher_model.parameters():
            param.requires_grad = False
        
        self.teacher_timestep_count = teacher_timestep_count  # e.g., 1024
        self.student_timestep_count = student_timestep_count  # e.g., 512

        # Sanity check
        assert teacher_timestep_count % student_timestep_count == 0, "teacher_t must be divisible by student_t"
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # Student random timestep
        student_timesteps = torch.randint(
            0, self.student_timestep_count, (bsz,), device=simplex.device, dtype=torch.int64
        )
        student_timesteps_scaled = scale(student_timesteps, self.student_timestep_count)

        # Corresponding teacher timestep
        teacher_timesteps = student_timesteps * (self.teacher_timestep_count // self.student_timestep_count)
        teacher_timesteps_scaled = scale(teacher_timesteps, self.teacher_timestep_count)

        # Forward diffusion for both
        noisy_simplex = self.noise_scheduler.add_noise(simplex, noise, student_timesteps)
        ...
        inputs_student = {
            ...
            "timesteps": student_timesteps_scaled,
            "classifier_free_guidance_in_train": self.classifier_free_guidance
        }
        with self.autocast_smart_context_manager():
            student_outputs = model(**inputs_student)
        ...
        with torch.no_grad():
            inputs_teacher = {
                ...
                "span_mask": inputs["span_mask"],
                "simplex": noisy_simplex,
                "timesteps": teacher_timesteps_scaled,
                "classifier_free_guidance_in_train": False,  # Teacher no guidance
            }
            teacher_outputs = self.teacher_model(**inputs_teacher)
            teacher_logits = teacher_outputs.logits

        # Distillation loss: MSE or KL
        if self.diffusion_args.distillation_loss_type == "mse":
            loss = F.mse_loss(student_logits, teacher_logits)
        elif self.diffusion_args.distillation_loss_type == "kl":
            loss = F.kl_div(
            ...
            raise ValueError(f"Unsupported distillation loss type: {self.diffusion_args.distillation_loss_type}")
