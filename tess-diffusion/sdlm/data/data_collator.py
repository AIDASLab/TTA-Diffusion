...
@dataclass
class CustomDataCollatorForPOS:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        ...
        pos_labels = [pos_label + [16]*(self.max_length - len(pos_label)) for pos_label in pos_labels]
        features["pos_labels"] = torch.tensor(pos_labels)

        return features

@dataclass
class CustomDataCollatorForLength:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        input_ids = [feature["input_ids"] for feature in features]
        #labels = [feature["labels"] for feature in features]
        span_mask = [feature["span_mask"] for feature in features]
        ...
        features["span_mask"] = torch.tensor(span_mask)

        return features

@dataclass
class CustomDataCollatorForTree:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        input_ids = [feature["input_ids"] for feature in features]
        #labels = [feature["labels"] for feature in features]
        parse_chart = [feature["parse_chart"] for feature in features]
        ...
        features["parse_chart"] = torch.tensor(parse_chart)

        return features

@dataclass
class CustomDataCollatorForSpan:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        input_ids = [feature["input_ids"] for feature in features]
        #labels = [feature["labels"] for feature in features]
        spans = [feature["spans"] for feature in features]
        ...
        features["spans"] = torch.tensor(spans)

        return features

@dataclass
class CustomDataCollatorForSentiment:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        ...
        features["sentiment"] = torch.tensor(sentiments)
        features["span_mask"] = torch.tensor(masks, dtype=torch.bool)

        return features

@dataclass
class CustomDataCollatorForTopic:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        ...
        features["topic"] = torch.tensor(topics)
        features["span_mask"] = torch.tensor(masks, dtype=torch.bool)

        return features

@dataclass
class CustomDataCollatorForToxicity:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features):
        input_ids = [feature["input_ids"] for feature in features]
        toxicity = [feature["toxicity"] for feature in features]
        ...
        features["toxicity"] = torch.tensor(toxicity)
        features["span_mask"] = torch.tensor(masks, dtype=torch.bool)
