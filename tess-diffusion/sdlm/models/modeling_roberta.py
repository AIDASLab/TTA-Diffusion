...
from transformers.models.bert.modeling_bert import BertModel, BertPreTrainedModel, BertOnlyMLMHead, BertLMPredictionHead
...
class BertForDiffusionLM(BertPreTrainedModel):
    ...
    def get_bert_empty_tokens(self, shape, device):
        if self.config.empty_token_be_mask:
            empty_token_ids = torch.ones(shape, dtype=torch.int64, device=device)*103
        else:
            # Padding token in bert-base is 0.
            empty_token_ids = torch.zeros(shape, dtype=torch.int64, device=device)
        empty_token_ids[:, 0] = 101
        empty_token_ids[:, -1] = 102
        return empty_token_ids
    ...
    def forward(
        ...
        if classifier_free_guidance or classifier_free_guidance_in_train:
            if self.config.classifier_free_simplex_inputs:
                if self.config.classifier_free_uncond_input == "empty_token":
                    empty_token_ids = self.get_bert_empty_tokens(shape=input_ids.shape, device=input_ids.device)
                    # TODO: fix the simplex_value later.
                    unconditional_simplex = convert_to_simplex(empty_token_ids, 5.0, self.config.vocab_size)
                elif self.config.classifier_free_uncond_input == "noisy_simplex":
                    simplex_shape = (input_ids.shape[0], input_ids.shape[1], self.config.vocab_size)
    ...
    def resize_position_embeddings(self, new_num_position_embeddings: int, with_alternatation=False):
        ...
        logger.info(f"Setting `config.max_position_embeddings={new_num_position_embeddings}`...")
        self.config.max_position_embeddings = new_num_position_embeddings
        old_position_embeddings_weight = self.bert.embeddings.position_embeddings.weight.clone()
        ...
        with torch.no_grad():
            if num_position_embeds_diff > 0:
                self.bert.embeddings.position_embeddings.weight[:-num_position_embeds_diff] = nn.Parameter(
                    old_position_embeddings_weight
                )
                if with_alternatation:
                    self.bert.embeddings.position_embeddings.weight[-num_position_embeds_diff:] = nn.Parameter(
        ...
        self.bert.embeddings.position_ids = (
            torch.arange(self.config.max_position_embeddings).expand((1, -1)).type_as(self.bert.embeddings.position_ids)
        )
        ...
        for i in range(self.config.num_hidden_layers):
            if (
                ...
                self.bert.encoder.layer[i].attention.self.distance_embedding = nn.Embedding(
                    2 * self.config.max_position_embeddings - 1, self.attention_head_size
                )
                old_distance_embedding_weight = self.layer[i].attention.self.distance_embedding.weight.clone()
                with torch.no_grad():
                    if num_position_embeds_diff > 0:
                        self.bert.encoder.layer[i].attention.self.distance_embedding.weight[
                            : -2 * num_position_embeds_diff
                        ] = nn.Parameter(old_distance_embedding_weight)
                    else:
                        self.bert.encoder.layer[i].attention.self.distance_embedding.weight = nn.Parameter(
                            old_distance_embedding_weight[: 2 * num_position_embeds_diff]
                        )
